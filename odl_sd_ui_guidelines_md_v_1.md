...
**Philosophy:** _Maximum MainPanel, minimum chrome._ The UI should feel invisible—neutral, fast, and predictable—while exposing deep power through context, traceability, and reversible actions.
...
4. **Role‑aware, phase‑aware.** Surfaces filter by capability and lifecycle gates; explain _why_ something is locked.
...
- **Show header** on hover/FocusWithin over top 48px strip _or_ click dots.
...
return (
  <div className="relative">
    ...
    />
    <header
      className={`fixed top-0 inset-x-0 transition-transform duration-200 ${headerOpen ? "translate-y-0" : "-translate-y-full"}`}
    >
      {/* header content */}
    </header>
    <button
      aria-label="Show header"
      className="absolute left-3 top-3 w-4 h-4 grid grid-cols-3 gap-[1px] opacity-80"
    >
      {Array.from({ length: 9 }).map((_, i) => (
        <span key={i} className="bg-gray-400/80 w-1 h-1 rounded" />
      ))}
    </button>
  </div>
);
...
<aside
  ...
  className={`transition-[width] duration-200 ${open ? "w-[250px]" : "w-[64px]"}`}
>
  ...
</aside>;
```
...
<aside
  className={`fixed right-0 top-0 h-full bg-white dark:bg-[var(--surface0)] transition-transform ${copilotOpen ? "translate-x-0" : "translate-x-full"}`}
...
<footer
  role="status"
  className="fixed bottom-0 inset-x-0 backdrop-blur bg-white/90 dark:bg-[var(--surface0)]/80 border-t border-[var(--border)] px-3 py-2"
>
