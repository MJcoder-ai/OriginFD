...
export class OriginFDClient {
  ...
  async listTasks(status?: string, limit = 50, offset = 0): Promise<Task[]> {
    ...
    return this.orchestratorApi.get("tasks/", { searchParams }).json<Task[]>();
  }
  ...
  async getToolMetadata(toolName: string): Promise<ToolMetadata> {
    return this.orchestratorApi.get(`tools/${toolName}`).json<ToolMetadata>();
  }
  ...
  async listComponents(category?: string): Promise<any[]> {
    const searchParams = category ? { category } : {};
    return this.api
      .get("marketplace/components", { searchParams })
      .json<any[]>();
  }
  ...
  async getPrimaryProjectDocument(projectId: string): Promise<OdlDocument> {
    return this.api
      .get(`projects/${projectId}/documents/primary`)
      .json<OdlDocument>();
  }
  ...
  async submitApproval(projectId: string, approved: boolean): Promise<any> {
    return this.api
      .post(`projects/${projectId}/review/approve`, { json: { approved } })
      .json();
  }
  ...
  async adoptScenario(projectId: string, scenarioId: string): Promise<any> {
    return this.api
      .post(`projects/${projectId}/scenarios/${scenarioId}/adopt`)
      .json();
  }
  ...
  async getTransactionHistory(tenantId: string): Promise<any[]> {
    return this.api
      .get(`transparency/tenants/${tenantId}/transactions`)
      .json<any[]>();
  }
  ...
  async patchViewOverrides(viewId: string, overrides: any): Promise<any> {
    return this.api
      .patch(`views/${viewId}/overrides`, { json: overrides })
      .json();
  }
  ...
  async createWarrantyClaim(
    ...
    return this.api
      .post(`components/${deviceInstanceId}/warranty/claims`, {
        json: formData,
      })
      .json();
  }
  ...
  async swapComponent(deviceInstanceId: string, swapData: any): Promise<any> {
    return this.api
      .post(`components/${deviceInstanceId}/swap`, { json: swapData })
      .json();
  }
