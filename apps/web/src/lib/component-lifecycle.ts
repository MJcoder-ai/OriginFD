...
export class ComponentLifecycleManager {
  ...
  private static readonly STATUS_METADATA = {
    draft: {
      label: "Draft",
      description:
        "Initial component record created, basic information captured",
      color: "gray",
      stage: "planning",
    ...
    enriched: {
      label: "Enriched",
      description:
        "Additional data enrichment completed, classifications assigned",
      color: "cyan",
      stage: "data_processing",
      required_actions: [
        "Initiate duplicate check",
    ...
    purchasing: {
      label: "Purchasing",
      description:
        "Purchase order in progress, awaiting supplier acknowledgment",
      color: "indigo",
      stage: "ordering",
    ...
    ordered: {
      label: "Ordered",
      description:
        "Order confirmed by supplier, production/manufacturing in progress",
      color: "blue",
      stage: "ordering",
    ...
    archived: {
      label: "Archived",
      description:
        "Component record archived, all lifecycle activities complete",
      color: "gray",
      stage: "archived",
  ...
  static canTransitionTo(
    currentStatus: ODLComponentStatus,
    targetStatus: ODLComponentStatus,
  ): {
    allowed: boolean;
  ...
  private static findTransitionPath(
    from: ODLComponentStatus,
    to: ODLComponentStatus,
    visited: Set<ODLComponentStatus> = new Set(),
    path: ODLComponentStatus[] = [],
  ): ODLComponentStatus[] {
    if (from === to) {
      return [...path, to];
    ...
    if (visited.has(from) || path.length > 10) {
    ...
    for (const nextStatus of this.getValidTransitions(from)) {
      const result = this.findTransitionPath(
        nextStatus,
        to,
        new Set(visited),
        currentPath,
