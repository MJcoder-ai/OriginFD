...
function deriveFromLibraries(document?: any): {
  ...
  const instances: ComponentInstance[] = libs.map((c: any, idx: number) => ({
    ...
    type:
      String(c.category || "").toLowerCase() === "pv_module"
        ? "pv_array"
        : String(c.category || "").toLowerCase() === "inverter"
          ? "inverter"
          : String(c.category || "").toLowerCase() === "battery"
            ? "battery"
...
export function SLDCanvas({
  ...
  const { instances, connections } = document?.instances?.length
    ? { instances: document.instances, connections: document.connections ?? [] }
    : deriveFromLibraries(document);

  // Filter by AC/DC layer selections
  const filtered = connections.filter(
    (c: any) =>
      (c.connection_type?.startsWith("ac")
        ? activeLayers.includes("ac")
        : true) &&
      (c.connection_type?.startsWith("dc")
        ? activeLayers.includes("dc")
        : true),
  );
  ...
  React.useEffect(() => {
    computeLayout(
      ...
      filtered.map((e: any) => ({
        ...
        connection_type: (e as any).connection_type,
      })),
      layoutEngine,
    ).then(setPositions);
      ...
      {React.createElement(SystemDiagram as unknown as any, {
        ...
        onPositionChange: async (
          ...
          try {
            ...
            ).apiClient.patchViewOverrides(`sld-${scope}`, posMap);
          } catch {}
        },
      })}
    </div>
