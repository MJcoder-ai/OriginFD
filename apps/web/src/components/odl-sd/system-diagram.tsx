...
import { Card, CardContent, CardHeader, CardTitle } from '@originfd/ui'

// Simple Badge component inline
const Badge = ({ children, className = "" }: { children: React.ReactNode; className?: string }) => (
  <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium border bg-gray-100 ${className}`}>
    {children}
  </span>
)
import { 
...
export function SystemDiagram({ instances, connections, className }: SystemDiagramProps) {
  ...
  const getComponentName = (id: string) => {
    ...
    return component?.type.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase()) || id
  }
  ...
  const getCapacityFromComponent = (id: string): string => {
    ...
    const capacity = component.parameters.capacity_kw || component.parameters.power_kw
    if (capacity) {
      if (capacity >= 1000) {
