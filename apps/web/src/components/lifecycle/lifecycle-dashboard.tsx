...
export default function LifecycleDashboard({
  ...
  const [selectedStatus, setSelectedStatus] =
    useState<ODLComponentStatus>("draft");
  ...
  const getLifecycleOverview = () => {
    ...
    return stageGroups.map((stage) => {
      const stageStatuses = ComponentLifecycleManager.getStatusesByStage(
        stage.stage,
      );
      const count = lifecycleData.filter((comp: ComponentResponse) =>
        stageStatuses.includes(
          comp.component_management?.status as ODLComponentStatus,
        ),
      ).length;
      ...
      return {
        ...
        count,
        percentage:
          lifecycleData.length > 0 ? (count / lifecycleData.length) * 100 : 0,
      };
    });
  };
  ...
  const filteredComponents =
    lifecycleData?.filter((comp: ComponentResponse) => {
      if (stageFilter !== "all") {
        const stageStatuses =
          ComponentLifecycleManager.getStatusesByStage(stageFilter);
        if (
          !stageStatuses.includes(
            comp.component_management?.status as ODLComponentStatus,
          )
      ...
      if (searchQuery) {
        ...
        return (
          comp.component_management?.component_identity?.name
            ?.toLowerCase()
            .includes(query) ||
          comp.component_management?.component_identity?.brand
            ?.toLowerCase()
            .includes(query) ||
          comp.component_management?.component_identity?.part_number
            ?.toLowerCase()
            .includes(query)
        );
      }
        ...
        <TabsContent value="components" className="space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {filteredComponents.map((component: ComponentResponse) => {
              const currentStatus = component.component_management
                ?.status as ODLComponentStatus;
              const statusDisplay = getStatusDisplay(currentStatus);
              const progress =
                ComponentLifecycleManager.getProgressPercentage(currentStatus);
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-center text-muted-foreground">
                ...
                <p className="text-sm">
                  This will show the complete ODL-SD v4.1 lifecycle flow
                </p>
              </div>
            </CardContent>
          </Card>
