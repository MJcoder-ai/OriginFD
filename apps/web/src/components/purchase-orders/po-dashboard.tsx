...
export default function PODashboard({
  ...
  const getStatusStats = () => {
    return {
      ...
      draft: purchaseOrders.filter((po: PurchaseOrder) => po.status === "draft")
        .length,
      pending_approval: purchaseOrders.filter(
        (po: PurchaseOrder) => po.status === "pending_approval",
      ).length,
      approved: purchaseOrders.filter(
        (po: PurchaseOrder) => po.status === "approved",
      ).length,
      in_production: purchaseOrders.filter(
        (po: PurchaseOrder) => po.status === "in_production",
      ).length,
      shipped: purchaseOrders.filter(
        (po: PurchaseOrder) => po.status === "shipped",
      ).length,
      delivered: purchaseOrders.filter(
        (po: PurchaseOrder) => po.status === "delivered",
      ).length,
      total_value: purchaseOrders.reduce(
        (sum: number, po: PurchaseOrder) => sum + po.pricing.total_amount,
        0,
      ),
    };
  };
  ...
  const getPendingApprovals = () => {
    return purchaseOrders.filter(
      (po: PurchaseOrder) =>
        ...
        po.approval_workflow.current_step <=
          po.approval_workflow.required_approvals.length,
    );
  };
                      ...
                      <TableCell>
                        <div>
                          <div className="font-medium">
                            {po.component_details.component_name}
                          </div>
                          <div className="text-sm text-muted-foreground">
                            {po.component_details.quantity_ordered.toLocaleString()}{" "}
                            {po.component_details.unit_of_measure}
                          </div>
                        </div>
                      </TableCell>
                      <TableCell>
                        <div>
                          <div className="font-medium">
                            {formatCurrency(
                              po.pricing.total_amount,
                              po.pricing.currency,
                            )}
                          </div>
                          <div className="text-sm text-muted-foreground">
                            {formatCurrency(
                              po.component_details.unit_price,
                              po.pricing.currency,
                            )}
                            /unit
                          </div>
                        ...
                        <div className="flex items-center space-x-2">
                                  ...
                                  <Card>
                                    ...
                                    <CardContent className="space-y-2 text-sm">
                                      ...
                                      <div>
                                        ...
                                        {po.pricing.payment_terms}
                                      </div>
                                      <div>
                                        <strong>Shipping Terms:</strong>{" "}
                                        {po.order_details.shipping_terms}
                                      </div>
                                    </CardContent>
                                  </Card>
                                </div>
                                  ...
                                  <CardContent>
                                    <div className="grid grid-cols-2 gap-4 text-sm">
                                      ...
                                      <div>
                                        ...
                                        {po.component_details.specification}
                                      </div>
                                      <div>
                                ...
                                <Card>
                                  ...
                                  <CardContent>
                                    <div className="space-y-3">
                                      {po.approval_workflow.required_approvals.map(
                                        (step, index) => {
                                          const StepIcon = getApprovalStepIcon(
                                            step.status,
                                          );
                                          return (
                                            <div
                                ...
                                <Card>
                                  ...
                                  <CardContent className="space-y-2 text-sm">
                                    <div className="flex justify-between">
                                      <span>Subtotal:</span>
                                      <span>
                                        {formatCurrency(po.pricing.subtotal)}
                                      </span>
                                    </div>
                                    <div className="flex justify-between">
                                      <span>Tax:</span>
                                      <span>
                                        {formatCurrency(po.pricing.tax_amount)}
                                      </span>
                                    </div>
                                    <div className="flex justify-between">
                                      <span>Shipping:</span>
                          ...
                          {po.status === "pending_approval" &&
                            (userRole === "procurement" ||
                              ...
                              <Dialog
                                open={
                                  approvalDialogOpen && selectedPO?.id === po.id
                                }
                              >
                                <DialogTrigger asChild>
                                  <Button
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            {po.supplier.name} •{" "}
                            {formatCurrency(po.pricing.total_amount)}
                          </p>
                        </div>
                        <Badge variant="secondary">
                          Step {po.approval_workflow.current_step} of{" "}
                          {po.approval_workflow.required_approvals.length}
                        </Badge>
                      </div>
                    </div>
