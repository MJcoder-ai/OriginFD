...
export default function RFQDashboard({
  ...
  const getStatusStats = () => {
    return {
      ...
      receiving_bids: rfqs.filter((r: any) => r.status === "receiving_bids")
        .length,
    ...
    return rfq.bids.reduce((best, current) =>
      current.evaluation_score && best.evaluation_score
        ? current.evaluation_score > best.evaluation_score
          ? current
          : best
        : current.total_price < best.total_price
          ? current
          : best,
    );
  };
  ...
  return (
    <div className="space-y-6">
                    ...
                    <TableCell>
                      ...
                      {rfq.bids.length > 0 && (
                        <div className="text-xs text-muted-foreground">
                          {
                            rfq.bids.filter((b) => b.status === "submitted")
                              .length
                          }{" "}
                          submitted
                        </div>
      ...
      {/* RFQ Creation Wizard */}
      <RFQCreationWizard open={createRFQOpen} onOpenChange={setCreateRFQOpen} />
    </div>
  );
}
