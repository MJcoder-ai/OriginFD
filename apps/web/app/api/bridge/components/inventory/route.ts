  ...
  componentIds.forEach((componentId) => {
    ...
    if (Math.random() > 0.3) {
      ...
      inventoryRecords.push({
        ...
        location_details: {
          building: warehouse.includes("Site")
            ? "Storage Building"
            : "Main Building",
...
function generateTransactionData() {
  ...
  for (let i = 0; i < 50; i++) {
    ...
    transactions.push({
      ...
      from_location:
        transactionType === "transfer" ? "Main Warehouse" : undefined,
      to_location:
        transactionType === "transfer" ? "Site Storage A" : undefined,
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (request.url.includes("/transactions")) {
      ...
      if (componentId) {
        filteredTransactions = mockTransactionData.filter(
          (t) => t.component_id === componentId,
        );
      }
    ...
    if (!mockInventoryData.length) {
      const componentIds = Array.from(
        { length: 115 },
        (_, i) => `comp_${String(i + 1).padStart(3, "0")}`,
      );
    ...
    if (componentId) {
      filteredInventory = filteredInventory.filter(
        (inv) => inv.component_id === componentId,
      );
    }

    if (warehouse) {
      filteredInventory = filteredInventory.filter(
        (inv) => inv.warehouse_location === warehouse,
      );
    }

    if (lowStock) {
      filteredInventory = filteredInventory.filter(
        (inv) => inv.quantity_available <= inv.reorder_level,
      );
    }
    ...
    console.log(
      `Fetching inventory: ${filteredInventory.length} records (page ${page})`,
    );
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (body.transaction_type) {
      ...
      const inventoryRecord = mockInventoryData.find(
        (inv) => inv.component_id === body.component_id,
      );
      if (inventoryRecord) {
        switch (body.transaction_type) {
          case "receipt":
            ...
            break;
          case "issue":
            inventoryRecord.quantity_available = Math.max(
              0,
              inventoryRecord.quantity_available - body.quantity,
            );
            break;
    ...
    console.log(
      "Created inventory record for component:",
      newInventory.component_id,
    );
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const inventoryIndex = mockInventoryData.findIndex(
      (inv) =>
