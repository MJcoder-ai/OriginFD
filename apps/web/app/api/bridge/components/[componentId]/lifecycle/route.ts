...
const LIFECYCLE_RULES: LifecycleValidationRule[] = [
  {
    ...
    trigger_events: ['technical_approval', 'specification_finalized', 'cancelled_by_user'],
  ...
  },
  {
    ...
    trigger_events: ['market_research_complete', 'supplier_identified', 'revision_required'],
  ...
  },
  {
    ...
    trigger_events: ['rfq_published', 'sourcing_cancelled', 'supplier_not_found'],
    user_roles_allowed: ['procurement', 'admin'],
  ...
  },
  {
    ...
    trigger_events: ['shipment_dispatched', 'order_cancelled', 'production_failed'],
  ...
  },
  {
    ...
    trigger_events: ['installation_started', 'damaged_in_transit', 'quality_failed', 'defect_found'],
  ...
  },
  {
    ...
    trigger_events: ['commissioning_started', 'installation_failed', 'component_failed'],
  ...
  },
  {
    ...
    trigger_events: ['system_activated', 'commissioning_failed', 'premature_failure'],
  ...
  },
  {
    ...
    trigger_events: ['warranty_period_started', 'maintenance_required', 'end_of_life'],
  ...
  },
  {
    ...
    trigger_events: ['warranty_claim', 'scheduled_maintenance', 'warranty_expired', 'warranty_replacement'],
    user_roles_allowed: ['operations', 'warranty', 'admin']
  },
  {
    from: 'maintenance',
    to: ['operational', 'retired', 'returned'],
    trigger_events: ['maintenance_complete', 'beyond_repair', 'replacement_required'],
...
function validateTransition(
  ...
  if (!rule.to.includes(toStatus)) {
    errors.push(`Invalid transition from ${fromStatus} to ${toStatus}. Allowed transitions: ${rule.to.join(', ')}`)
    return { valid: false, errors, warnings }
  }

  // Check if trigger event is valid
  if (!rule.trigger_events.includes(triggerEvent)) {
    errors.push(`Invalid trigger event: ${triggerEvent}. Allowed events: ${rule.trigger_events.join(', ')}`)
    return { valid: false, errors, warnings }
  }

  // Check user role permissions
  if (!rule.user_roles_allowed.includes(userRole)) {
    errors.push(`User role ${userRole} not authorized for this transition. Allowed roles: ${rule.user_roles_allowed.join(', ')}`)
    return { valid: false, errors, warnings }
  }
...
function getNextPossibleStates(currentStatus: ODLComponentStatus): ODLComponentStatus[] {
...
export async function GET(
  ...
  try {
    ...
    const currentStatus = searchParams.get('current_status') as ODLComponentStatus

    if (!currentStatus) {
      return NextResponse.json({ error: 'current_status parameter is required' }, { status: 400 })
    }
    ...
    const response = {
      ...
      available_transitions: rule ? rule.trigger_events.map(event => ({
        trigger_event: event,
      ...
      lifecycle_rules: {
        ...
        automatic_transitions: LIFECYCLE_RULES.filter(r => r.automatic).length
      }
    }
