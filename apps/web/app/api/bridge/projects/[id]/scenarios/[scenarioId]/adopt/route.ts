...
export async function POST(
  ...
  const filePath = path.join(process.cwd(), 'services', 'orchestrator', 'planner', 'mock_scenarios.json')
  ...
  await fetch(`${origin}/api/notifications`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ projectId: id, scenarioId, message: `Scenario ${scenarioId} adopted` }),
  })
