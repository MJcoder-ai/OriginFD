...
export async function POST(
  ...
  try {
    ...
    if (approvalData.action === "reject") {
      ...
      const hasMoreSteps = mockPO.approval_workflow.required_approvals.some(
        (step) => step.step_number > mockPO.approval_workflow.current_step,
      );
      ...
      approval_workflow: {
        ...
        approved_by:
          approvalData.action === "approve"
            ...
            : (mockPO.approval_workflow as any).approved_by,
        approved_at:
          (overallStatus as any) === "approved"
            ? new Date().toISOString()
            : undefined,
        rejection_reason:
          approvalData.action === "reject" ? approvalData.notes : undefined,
      },
      next_actions: getNextActions(newStatus, overallStatus),
    };

    console.log(
      `PO ${mockPO.po_number} ${approvalData.action} by ${approvalData.approver_role}`,
    );
