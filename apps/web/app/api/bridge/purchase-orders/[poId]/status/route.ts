...
export async function PATCH(
  ...
  try {
    ...
    const componentImpact = getComponentLifecycleImpact(
      newStatus,
      mockPO.component_id,
    );

    // Determine notifications
    const notifications = getStatusChangeNotifications(
      oldStatus,
      newStatus,
      mockPO,
    );
    ...
    console.log(
      `PO ${mockPO.po_number} status updated: ${oldStatus} → ${newStatus}`,
    );
...
function getStatusChangeNotifications(
  oldStatus: POStatus,
  newStatus: POStatus,
  po: any,
) {
