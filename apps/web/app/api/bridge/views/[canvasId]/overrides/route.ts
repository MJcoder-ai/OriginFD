...
export async function GET(_req: NextRequest, { params }: { params: { canvasId: string } }) {
  ...
  try {
    ...
    for (const r of rows) map[r.instanceId] = { x: r.x, y: r.y, rotation: r.rotation ?? undefined }
...
export async function PATCH(req: NextRequest, { params }: { params: { canvasId: string } }) {
  const { canvasId } = params
  try {
    const body = await req.json().catch(() => ({})) as Record<string, { x: number; y: number; rotation?: number }>
    ...
    const ops = Object.entries(body).map(([instanceId, p]) =>
      prisma.viewOverride.upsert({
        ...
        create: { canvasId, instanceId, x: p.x, y: p.y, rotation: p.rotation ?? null },
