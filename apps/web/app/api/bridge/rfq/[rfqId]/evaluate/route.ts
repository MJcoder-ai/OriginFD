...
function calculateBidScore(bid: RFQBid, criteria: EvaluationCriteria, allBids: RFQBid[]): BidEvaluation {
  ...
  const deliveryScore = latestDelivery === earliestDelivery ? 100 :
    ((latestDelivery - bidDeliveryTime) / (latestDelivery - earliestDelivery)) * 100

  // Quality scoring (based on certifications and compliance)
  const complianceRate = bid.specifications_compliance.filter(s => s.compliant).length /
  ...
  const sustainabilityScore = bid.sustainability_score || 60 + Math.random() * 40
...
export async function POST(
  ...
  try {
    ...
    const totalWeight = Object.values(evaluationData.criteria).reduce((sum, weight) => sum + weight, 0) - evaluationData.criteria.total_weight
    ...
    const evaluationResult = {
      ...
      summary: {
        ...
        recommended_awards: evaluations.filter(e => e.recommendation === 'award').length,
        shortlisted: evaluations.filter(e => e.recommendation === 'shortlist').length,
        rejected: evaluations.filter(e => e.recommendation === 'reject').length,
