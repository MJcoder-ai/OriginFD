...
const WORKFLOW_STAGES: WorkflowStep[] = [
  {
    ...
    description: 'Initial component specification and technical requirements definition',
    required_actions: [
  ...
  },
  {
    ...
    description: 'Active sourcing and supplier engagement for component procurement',
    required_actions: [
  ...
  },
  {
    stage: 'rfq_awarded',
    name: 'Supplier Award',
    description: 'RFQ evaluation complete and contract awarded to selected supplier',
    required_actions: [
  ...
  },
  {
    ...
    key_stakeholders: ['Installation Team', 'Engineering', 'Project Management'],
    prerequisites: [
      'Site readiness',
  ...
  },
  {
    ...
    description: 'Component under active warranty coverage with performance monitoring',
    required_actions: [
  ...
  },
  {
    stage: 'retired',
    name: 'Retirement',
    description: 'Component end-of-life and preparation for disposal or recycling',
    required_actions: [
  ...
  },
  {
    ...
    description: 'Component lifecycle documentation archived for future reference',
    required_actions: [
...
function findWorkflowStage(status: ODLComponentStatus): WorkflowStep | undefined {
  return WORKFLOW_STAGES.find(stage => stage.stage === status)
}

function calculateWorkflowCompletion(currentStatus: ODLComponentStatus): number {
  const currentIndex = WORKFLOW_STAGES.findIndex(stage => stage.stage === currentStatus)
  return currentIndex >= 0 ? Math.round(((currentIndex + 1) / WORKFLOW_STAGES.length) * 100) : 0
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const nextPossibleStages = getNextPossibleStages(validationRequest.current_status)
    const workflowCompletion = calculateWorkflowCompletion(validationRequest.current_status)
    ...
    switch (validationRequest.current_status) {
      case 'draft':
        recommendations.push('Ensure all technical specifications are complete before approval')
      ...
      case 'sourcing':
        ...
        recommendations.push('Define clear evaluation criteria and scoring methodology')
        break
      case 'operational':
      ...
      case 'retired':
        ...
        recommendations.push('Archive all documentation for compliance requirements')
        break
    }
    ...
    currentStage.prerequisites.forEach(prerequisite => {
      ...
      if (Math.random() > 0.8) { // Randomly flag some prerequisites as missing for demo
...
export async function GET(request: NextRequest) {
  try {
    ...
    const workflowOverview = {
      ...
      automation_summary: {
        manual: WORKFLOW_STAGES.filter(s => s.automation_level === 'manual').length,
        semi_automated: WORKFLOW_STAGES.filter(s => s.automation_level === 'semi-automated').length,
        automated: WORKFLOW_STAGES.filter(s => s.automation_level === 'automated').length
      },
      stakeholder_involvement: WORKFLOW_STAGES.reduce((acc, stage) => {
