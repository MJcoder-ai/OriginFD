...
const mockMediaAssets: MediaAsset[] = [
  ...
  },
  {
    ...
    description:
      "Technical datasheet for ABB TRIO-50.0-TL-OUTD String Inverter",
  ...
  try {
    ...
    if (componentId) {
      filteredAssets = filteredAssets.filter(
        (asset) => asset.component_id === componentId,
      );
    }
    ...
    const assetsByComponent = filteredAssets.reduce(
      (acc, asset) => {
    ...
    const response = {
      ...
      statistics: {
        by_type: mockMediaAssets.reduce(
          (acc, asset) => {
        ...
        total_size: mockMediaAssets.reduce(
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      "Created new media asset:",
      newAsset.id,
      "for component:",
      newAsset.component_id,
    );
    ...
    return NextResponse.json(
      {
        ...
        upload_info: {
          ...
          searchable:
            newAsset.type === "datasheet" || newAsset.type === "certificate",
        },
      },
      { status: 201 },
    );
