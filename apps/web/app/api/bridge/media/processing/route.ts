...
const mockProcessingJobs: MediaProcessingJob[] = [
  {
    ...
    output_metadata: {
      ...
      extracted_entities: ['power_rating', 'voltage', 'efficiency', 'dimensions']
    }
  },
  {
    id: 'job_002',
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!jobRequest.asset_id || !jobRequest.job_types || jobRequest.job_types.length === 0) {
    ...
    console.log(`Created ${createdJobs.length} processing jobs for asset ${jobRequest.asset_id}`)
...
function generateMockOutput(jobType: ProcessingJobType): Record<string, any> {
  switch (jobType) {
    ...
    case 'data_extraction':
      return {
        extracted_entities: ['power_rating', 'efficiency', 'dimensions', 'warranty'],
