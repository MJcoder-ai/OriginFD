...
export default function TransparencyDashboardPage() {
  ...
  const totalFees = React.useMemo(() => {
    return tx?.transactions.reduce((sum: number, t: any) => sum + (t.fee || 0), 0) || 0
  }, [tx])

  const totalSavings = React.useMemo(() => {
    return psu?.events.reduce((sum: number, e: any) => sum + (e.savings || 0), 0) || 0
  }, [psu])

  const totalCarbon = React.useMemo(() => {
    return psu?.events.reduce((sum: number, e: any) => sum + (e.carbon || 0), 0) || 0
  }, [psu])
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          <CardHeader><CardTitle>PSU Consumption</CardTitle></CardHeader>
          <CardContent>
            <p className="text-2xl font-bold">{psu?.total_psu ?? 0}</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader><CardTitle>Fees Accrued</CardTitle></CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader><CardTitle>Carbon Impact</CardTitle></CardHeader>
          <CardContent>
