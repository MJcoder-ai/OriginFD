...
class PSUBudgetExceeded(Exception):
    ...
    def __init__(self, requested: int, available: int, tenant_id: str):
        ...
        super().__init__(
            f"PSU budget exceeded: requested {requested}, available {available}"
        )
...
class PolicyRouter:
    ...
    def __init__(self):
        ...
        self.role_permissions = {
            ...
            "viewer": ["design_read", "component_read", "finance_read"],
        }
    ...
    def register_task_start(self, tenant_id: Optional[str], user_id: Optional[str]):
        ...
        if tenant_id:
            self.active_tasks_by_tenant[tenant_id] = (
                self.active_tasks_by_tenant.get(tenant_id, 0) + 1
            )
        if user_id:
    ...
    def register_task_completion(
        self, tenant_id: Optional[str], user_id: Optional[str]
    ):
        """Register the completion of a task for concurrency tracking."""
        if tenant_id and tenant_id in self.active_tasks_by_tenant:
            self.active_tasks_by_tenant[tenant_id] = max(
                0, self.active_tasks_by_tenant[tenant_id] - 1
            )
        ...
        if user_id:
            ...
            if key in self.active_tasks_by_user:
                self.active_tasks_by_user[key] = max(
                    0, self.active_tasks_by_user[key] - 1
                )
            ...
            if not budget_check["approved"]:
                violations.append(
                    PolicyViolation(
                        ...
                        metadata={
                            "requested_psu": estimated_psu_cost,
                            "available_psu": budget_check.get("available", 0),
                        },
                    )
                )
    ...
    async def set_rate_limit(
        ...
        logger.info(
            f"Set rate limit: {resource_type} = {limit_count}/{time_window_seconds}s for {key}"
        )

    async def consume_psu_budget(
        self, tenant_id: str, actual_psu_cost: int, task_id: str
    ):
        ...
        if tenant_id in self.budget_allocations:
            ...
            allocation.reserved_budget = max(
                0, allocation.reserved_budget - actual_psu_cost
            )
            ...
            logger.debug(
                f"Consumed {actual_psu_cost} PSU for tenant {tenant_id} (task: {task_id})"
            )
        else:
            logger.warning(f"No budget allocation found for tenant {tenant_id}")

    async def release_psu_reservation(
        self, tenant_id: str, reserved_psu: int, task_id: str
    ):
        """Release PSU reservation if task is cancelled."""
        if tenant_id in self.budget_allocations:
            allocation = self.budget_allocations[tenant_id]
            allocation.reserved_budget = max(
                0, allocation.reserved_budget - reserved_psu
            )

            logger.debug(
                f"Released {reserved_psu} PSU reservation for tenant {tenant_id} (task: {task_id})"
            )
    ...
    async def get_budget_status(self, tenant_id: str) -> Dict[str, Any]:
        ...
        available = (
            ...
            - allocation.used_budget
            - allocation.reserved_budget
        )
        ...
        return {
            ...
            "utilization_percent": (allocation.used_budget / allocation.total_budget)
            * 100,
    ...
    async def _check_psu_budget(
        self, tenant_id: Optional[str], user_id: Optional[str], requested_psu: int
    ) -> Dict[str, Any]:
    ...
    async def _check_rate_limits(
        self, tenant_id: Optional[str], user_id: Optional[str], resource_type: str
    ) -> Dict[str, Any]:
    ...
    async def _check_resource_limits(
        self, estimated_duration_ms: int, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if tenant_id:
            ...
            if (
                tenant_active
                >= self.default_policies["max_concurrent_tasks_per_tenant"]
            ):
                return {
                    "approved": False,
    ...
    async def _check_content_policy(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for term in banned_terms:
            if term in text:
                return {
                    "approved": False,
                    "reason": f"Content contains banned term: {term}",
                }
    ...
    async def _reserve_psu_budget(
        self, tenant_id: Optional[str], user_id: Optional[str], psu_amount: int
    ):
    ...
    async def _generate_modifications(
        self, violations: List[PolicyViolation], context: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        for violation in violations:
            if violation.violation_type == PolicyViolationType.BUDGET_EXCEEDED:
                ...
                violation.violation_type == PolicyViolationType.RESOURCE_LIMIT_EXCEEDED
            ):
                # Suggest reducing task scope
                modifications["max_duration_ms"] = (
                    self.default_policies["max_task_duration_minutes"] * 60 * 1000
                )
