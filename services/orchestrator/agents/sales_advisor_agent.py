...
class SalesAdvisorAgent(BaseAgent):
    ...
    async def create_plan(self, task: str, context: AgentContext) -> AgentPlan:
        ...
        elif "customer" in task_lower and (
            "analyz" in task_lower or "segment" in task_lower
        ):
    ...
    async def execute_plan(self, plan: AgentPlan, context: AgentContext) -> AgentResult:
        ...
        try:
            ...
            for i, step in enumerate(plan.steps):
                ...
                if step.get("tool_name"):
                    ...
                    if not tool_result.success:
                        ...
                        if (
                            "financial" in step["tool_name"].lower()
                            or "roi" in step["tool_name"].lower()
                        ):
                            fallback_result = await self._execute_financial_fallback(
                                step, context
                            )
                            execution_results.append(fallback_result)
                else:
                    # Non-tool step (analysis, synthesis, etc.)
                    step_result = await self._execute_sales_analysis_step(
            ...
            quality_score = await self._calculate_sales_quality_score(
            ...
            result = AgentResult(
                ...
                next_actions=await self._suggest_sales_next_actions(
                    final_result, context
                ),
            )

            logger.info(
                f"Sales advisory plan executed successfully in {execution_time}ms"
            )
            return result
    ...
    async def _plan_financial_analysis(
        ...
        steps = [
            ...
            },
            {
                ...
                "inputs": {
                    ...
                    "inflation_rate": context.current_state.get(
    ...
    async def _plan_proposal_generation(
        self, task: str, context: AgentContext
    ) -> List[Dict[str, Any]]:
        ...
        steps = [
            ...
            },
            {
                ...
                "inputs": {
                    ...
                    "pricing_strategy": context.current_state.get(
            ...
            },
            {
                ...
                "inputs": {
                    ...
                    "template_type": context.current_state.get(
    ...
    async def _plan_incentive_optimization(
        self, task: str, context: AgentContext
    ) -> List[Dict[str, Any]]:
        ...
        steps = [
            {
                ...
                "inputs": {
                    ...
                    "customer_type": context.current_state.get(
            ...
            },
            {
                ...
                "inputs": {
                    ...
                    "project_parameters": context.current_state.get(
                        "project_parameters"
                    ),
                    "optimization_goal": context.current_state.get(
                        "optimization_goal", "maximize_savings"
                    ),
                },
                "estimated_duration_ms": 2500,
                "estimated_cost_psu": 5,
    ...
    async def _plan_competitive_analysis(
        self, task: str, context: AgentContext
    ) -> List[Dict[str, Any]]:
        ...
        steps = [
            {
                ...
                "inputs": {
                    "market_segment": context.current_state.get(
            ...
            },
            {
                ...
                "inputs": {
                    ...
                    "value_propositions": context.current_state.get(
                        "value_propositions"
                    ),
                },
                "estimated_duration_ms": 3000,
    ...
    async def _plan_customer_analysis(
        self, task: str, context: AgentContext
    ) -> List[Dict[str, Any]]:
        ...
        steps = [
            {
                ...
                "inputs": {
                    "customer_data": context.current_state.get("customer_data"),
                    "interaction_history": context.current_state.get(
                        "interaction_history"
                    ),
            ...
            },
            {
                ...
                "inputs": {
                    ...
                    "segmentation_criteria": context.current_state.get(
            ...
            },
            {
                ...
                "inputs": {
                    ...
                    "available_solutions": context.current_state.get(
                        "available_solutions"
                    ),
                },
                "estimated_duration_ms": 2500,
    ...
    async def _plan_general_sales_task(
        self, task: str, context: AgentContext
    ) -> List[Dict[str, Any]]:
    ...
    async def _execute_sales_analysis_step(
        ...
        if (
            "gather" in step_description.lower()
            and "financial" in step_description.lower()
        ):
            ...
            return {
                ...
                "content": {
                    "financial_parameters": {
                        ...
                        "annual_savings": context.current_state.get(
                            "annual_savings", 0
                        ),
                        "system_lifetime": context.current_state.get(
                            "system_lifetime", 25
                        ),
                        "discount_rate": self.default_discount_rate,
                        "analysis_ready": True,
            ...
            "analyze" in step_description.lower()
            and "sales" in step_description.lower()
        ):
            ...
            return {
                ...
                "content": {
                    "sales_analysis": {
                        "objectives": context.current_state.get(
                            "objectives", ["maximize_roi"]
                        ),
                        "constraints": context.current_state.get("constraints", []),
                        "customer_priorities": context.current_state.get(
                            "customer_priorities", ["cost", "quality"]
                        ),
            ...
            # Generate report
            report_type = (
                "financial" if "financial" in step_description.lower() else "sales"
            )
            ...
            for result in previous_results:
                if hasattr(result, "content") and result.content:
                    ...
                    if isinstance(content, dict):
                        if (
                            "roi_percentage" in content
                            and content["roi_percentage"] > self.min_roi_threshold * 100
                        ):
                            recommendations.append(
                                "Project shows strong financial returns"
                            )
                        if (
                            "payback_years" in content
                            and content.get("payback_years", 999) < 10
                        ):
                            recommendations.append(
                                "Attractive payback period for customer"
                            )
                        if "incentives" in content:
                            recommendations.append(
                                "Leverage available incentives to improve value proposition"
                            )

            if not recommendations:
                recommendations = [
                    "Review project parameters and optimize for customer needs"
                ]
        ...
        for result in execution_results:
            if hasattr(result, "content") and result.content:
                ...
                if isinstance(content, dict):
                    ...
                    if any(
                        key in content
                        for key in ["roi_percentage", "payback_years", "net_savings"]
                    ):
                        ...
                        synthesis["key_metrics"]["financial_viability"] = (
                            "strong"
                            if content.get("roi_percentage", 0) > 15
                            else "moderate"
                        )
                    ...
                    if "incentives" in content or "rebates" in content:
                        ...
                        if "total_incentive_value" in content:
                            synthesis["key_metrics"]["incentive_value"] = content[
                                "total_incentive_value"
                            ]

                    # Extract recommendations
                    if "recommendations" in content:
                        synthesis["sales_recommendations"].extend(
                    ...
                    if "quote" in content or "proposal" in content:
                        ...
                        if "total_price" in content:
                            synthesis["key_metrics"]["quoted_price"] = content[
                                "total_price"
                            ]

                    # Extract competitive analysis
                    if "competitive" in content:
                        synthesis["key_metrics"]["competitive_position"] = content.get(
                            "position", "competitive"
                        )

        # Generate sales-specific recommendations
        if "roi" in task_description.lower() or "financial" in task_description.lower():
            synthesis["sales_recommendations"].append(
                "Present financial benefits clearly to customer"
            )
            synthesis["next_steps"].append(
                "Schedule financial review meeting with customer"
            )

        if "proposal" in task_description.lower():
            synthesis["sales_recommendations"].append(
                "Customize proposal based on customer priorities"
            )
            synthesis["next_steps"].append("Deliver proposal and schedule follow-up")

        if "incentive" in task_description.lower():
            synthesis["sales_recommendations"].append(
                "Highlight time-sensitive incentive opportunities"
            )
            synthesis["next_steps"].append(
                "Assist customer with incentive applications"
            )
    ...
    async def _calculate_sales_quality_score(
        ...
        financial_bonus = (
            0.1
            if final_result.get("key_metrics", {}).get("financial_viability")
            == "strong"
            else 0
        )
    ...
    async def _calculate_plan_confidence(
        ...
        if context.current_state.get("project_cost") and context.current_state.get(
            "annual_savings"
        ):
            base_confidence += 0.05
    ...
    async def _generate_plan_reasoning(
        ...
        reasoning_parts = [f"Created {len(steps)}-step sales advisory plan for: {task}"]
        ...
        if tool_steps:
            reasoning_parts.append(
                f"Plan includes {len(tool_steps)} specialized tool executions"
            )

        if analysis_steps:
            reasoning_parts.append(
                f"Plan includes {len(analysis_steps)} analysis and synthesis steps"
            )

        # Mention specific capabilities
        if any(
            "financial" in s["description"].lower() or "roi" in s["description"].lower()
            for s in steps
        ):
            reasoning_parts.append(
                "Financial analysis capabilities will provide ROI insights"
            )

        if any(
            "proposal" in s["description"].lower()
            or "quote" in s["description"].lower()
            for s in steps
        ):
            reasoning_parts.append(
                "Proposal generation capabilities will create customized offers"
            )

        if any("incentive" in s["description"].lower() for s in steps):
            reasoning_parts.append(
                "Incentive optimization will maximize customer value"
            )

        if any("competitive" in s["description"].lower() for s in steps):
            reasoning_parts.append(
                "Competitive analysis will inform positioning strategy"
            )
