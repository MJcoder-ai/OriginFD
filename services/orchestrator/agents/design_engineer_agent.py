...
class DesignEngineerAgent(BaseAgent):
    ...
    def __init__(
        ...
        capabilities = [
            ...
            AgentCapability(
                ...
                tools=["OptimizeLayoutTool", "OptimizeCostTool", "SuggestImprovementsTool"],
    ...
    async def create_plan(self, task: str, context: AgentContext) -> AgentPlan:
        ...
        if "validate" in task_lower and ("odl" in task_lower or "document" in task_lower):
        ...
        try:
            ...
            for i, step in enumerate(plan.steps):
                ...
                if step.get("tool_name"):
                    ...
                    step_result = await self._execute_analysis_step(step, context, execution_results)
            ...
            quality_score = await self._calculate_quality_score(execution_results, final_result)
            ...
            logger.info(f"Design engineering plan executed successfully in {execution_time}ms")
            return result
    ...
    async def _plan_odl_validation(self, task: str, context: AgentContext) -> List[Dict[str, Any]]:
        ...
        steps = [
            ...
            },
            {
                ...
                "inputs": {
                    ...
                    "standards": context.current_state.get("compliance_standards", ["NEC", "IEEE"])
                },
    ...
    async def _plan_energy_simulation(self, task: str, context: AgentContext) -> List[Dict[str, Any]]:
        ...
        steps = [
            ...
            },
            {
                ...
                "inputs": {
                    ...
                    "simulation_period": context.current_state.get("simulation_period", "annual")
                },
    ...
    async def _plan_design_optimization(self, task: str, context: AgentContext) -> List[Dict[str, Any]]:
        ...
        steps = [
            ...
            },
            {
                ...
                "inputs": {
                    ...
                    "optimization_goals": context.current_state.get("optimization_goals", ["performance", "cost"])
                },
            ...
            },
            {
                ...
                "inputs": {
                    ...
                    "budget_constraints": context.current_state.get("budget_constraints")
                },
    ...
    async def _plan_component_analysis(self, task: str, context: AgentContext) -> List[Dict[str, Any]]:
        ...
        steps = [
            ...
            },
            {
                ...
                "inputs": {
                    ...
                    "system_requirements": context.current_state.get("system_requirements")
                },
                "estimated_duration_ms": 2500,
                "estimated_cost_psu": 4
            },
    ...
    async def _plan_general_design_task(self, task: str, context: AgentContext) -> List[Dict[str, Any]]:
        ...
        steps = [
            ...
            },
            {
                ...
                "tool_name": "ValidateOdlTool",
                "inputs": {
                    "document": context.current_state.get("design_data")
                },
                "estimated_duration_ms": 3000,
                "estimated_cost_psu": 5
            },
                ...
                if hasattr(result, 'content') and result.content:
                    if isinstance(result.content, dict):
                        if "performance" in result.content:
                            analysis_result["key_findings"].append("Performance data analyzed")
                        if "efficiency" in result.content:
                            analysis_result["key_findings"].append("Efficiency metrics extracted")
                ...
                if isinstance(content, dict):
                    ...
                    if "validation" in str(content).lower():
                        ...
                        if "compliance" in content:
                            synthesis["technical_details"]["compliance_status"] = content.get("compliance")

                    # Extract simulation results
                    if "simulation" in str(content).lower() or "performance" in str(content).lower():
                        synthesis["key_findings"].append("Performance simulation completed")
                        if "energy_output" in content:
                            synthesis["technical_details"]["energy_performance"] = content.get("energy_output")

                    # Extract optimization results
                    if "optimization" in str(content).lower() or "improvement" in str(content).lower():
                        synthesis["key_findings"].append("Design optimization completed")
                        if "improvements" in content:
                            synthesis["recommendations"].extend(content.get("improvements", []))

        # Generate recommendations based on task type
        if "optimize" in task_description.lower():
            synthesis["recommendations"].append("Review optimization suggestions and implement high-impact changes")
            synthesis["next_steps"].append("Run validation tests on optimized design")

        if "validate" in task_description.lower():
            synthesis["recommendations"].append("Address any compliance issues identified")
            synthesis["next_steps"].append("Update design documentation with validation results")

        if "simulate" in task_description.lower():
            synthesis["recommendations"].append("Analyze performance metrics against requirements")
            synthesis["next_steps"].append("Consider optimization opportunities identified")
    ...
    async def _calculate_quality_score(
        ...
        # Completeness score
        completeness = 1.0 if final_result.get("key_findings") and final_result.get("recommendations") else 0.7

        # Weighted final score
        quality_score = (success_rate * 0.4) + (avg_confidence * 0.4) + (completeness * 0.2)
    ...
    async def _calculate_plan_confidence(
        ...
        required_tools = [step.get("tool_name") for step in steps if step.get("tool_name")]
    ...
    async def _generate_plan_reasoning(self, task: str, steps: List[Dict[str, Any]]) -> str:
        ...
        if tool_steps:
            reasoning_parts.append(f"Plan includes {len(tool_steps)} tool executions for technical analysis")

        if analysis_steps:
            reasoning_parts.append(f"Plan includes {len(analysis_steps)} analysis steps for synthesis and recommendations")

        # Mention specific capabilities used
        if any("validate" in s["description"].lower() for s in steps):
            reasoning_parts.append("Validation capabilities will ensure design compliance")

        if any("simulate" in s["description"].lower() for s in steps):
            reasoning_parts.append("Simulation capabilities will provide performance insights")

        if any("optimize" in s["description"].lower() for s in steps):
            reasoning_parts.append("Optimization capabilities will identify improvement opportunities")
