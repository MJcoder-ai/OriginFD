...
class BaseAgent(ABC):
    ...
    async def validate_plan(self, plan: AgentPlan, context: AgentContext) -> Dict[str, Any]:
        """
    ...
    async def communicate_with_agent(
        ...
        handover_message = {
            ...
            "shared_scratchpad": self.current_context.shared_scratchpad if self.current_context else {}
        }

        # Store handover in episodic memory
        await self.episodic_memory.store_interaction(
            session_id=self.current_context.session_id if self.current_context else "system",
            agent_id=self.agent_id,
    ...
    async def get_capability_assessment(self, task_description: str) -> Dict[str, Any]:
        ...
        overall_confidence = overall_confidence / len(self.capabilities) if self.capabilities else 0.0
        ...
        return {
            ...
            "reasoning": await self._generate_capability_reasoning(task_analysis, capability_scores)
        }
    ...
    def _update_performance_metrics(self, result: AgentResult):
        ...
        self.average_execution_time_ms = (
            (self.average_execution_time_ms * (self.total_executions - 1) + result.execution_time_ms)
