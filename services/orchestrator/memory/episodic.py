...
class MemoryRecord(BaseModel):
    ...
    timestamp: datetime
    interaction_type: str  # "user_message", "agent_response", "tool_execution", "handover", etc.
...
class EpisodicMemory:
    ...
    async def close_session(self, session_id: str, summary: Optional[str] = None):
        ...
        if session_id in self.active_sessions:
            ...
            session.summary = summary or await self._generate_session_summary(session_id)
    ...
    async def _create_tables(self):
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            await db.execute("CREATE INDEX IF NOT EXISTS idx_user_id ON memory_records(user_id)")
            await db.execute("CREATE INDEX IF NOT EXISTS idx_timestamp ON memory_records(timestamp)")
            await db.execute("CREATE INDEX IF NOT EXISTS idx_interaction_type ON memory_records(interaction_type)")
    ...
    async def _update_session_summary(self, session_id: str, record: MemoryRecord):
        ...
        keywords = ["component", "design", "project", "simulation", "optimization", "procurement"]
