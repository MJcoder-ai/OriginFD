...
class FunctionalRequirements(BaseModel):
    ...
    capacity_kw: Optional[float] = Field(default=None, description="System capacity in kW", ge=0)
    annual_generation_kwh: Optional[float] = Field(default=None, description="Annual energy generation in kWh", ge=0)
...
class TechnicalRequirements(BaseModel):
    ...
    voltage_level: Optional[str] = Field(None, description="Voltage level specification")
    frequency_hz: Optional[float] = Field(None, description="System frequency in Hz", ge=0)


class RegulatoryRequirements(BaseModel):
    """Regulatory requirements"""
    grid_codes: List[str] = Field(default_factory=list, description="Applicable grid codes")
    safety_standards: List[str] = Field(default_factory=list, description="Safety standards")
    environmental_permits: List[str] = Field(default_factory=list, description="Environmental permits")
...
class DataManagement(BaseModel):
    ...
    partitioning_enabled: bool = Field(default=False, description="Enable data partitioning")
    external_refs_enabled: bool = Field(default=False, description="Enable external references")
    streaming_enabled: bool = Field(default=False, description="Enable streaming data")
    max_document_size_mb: int = Field(default=100, description="Maximum document size in MB", ge=1)
...
class ComponentInstance(BaseModel):
    ...
    parameters: Dict[str, Any] = Field(default_factory=dict, description="Instance parameters")
...
class OdlSdDocument(BaseModel):
    ...
    hierarchy: Dict[str, Any] = Field(default_factory=dict, description="System hierarchy")
    requirements: Requirements = Field(default_factory=Requirements, description="System requirements")
    libraries: Dict[str, Any] = Field(default_factory=dict, description="Component libraries")
    instances: List[ComponentInstance] = Field(default_factory=list, description="Component instances")
    connections: List[Connection] = Field(default_factory=list, description="System connections")
