...
# FastAPI app
app = FastAPI(
    title="OriginFD API", description="Simplified Database Integration", version="0.3.1"
)
...
async def list_projects(
    ...
    """List all projects (requires admin or engineer role)"""
    user_roles = current_user.get("roles", [])
    if "admin" not in user_roles and "engineer" not in user_roles:
        raise HTTPException(
...
async def get_project(
    ...
    """Get specific project (authentication required)"""
...
async def create_project(
    ...
    """Create new project (requires admin or engineer role)"""
    user_roles = current_user.get("roles", [])
    if "admin" not in user_roles and "engineer" not in user_roles:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Insufficient permissions"
...
async def get_document(document_id: str, db: Session = Depends(get_db)):
    ...
    return {
        ...
        "content": {
            ...
            or f"{project.domain} {project.scale} project",
            "capacity": (
