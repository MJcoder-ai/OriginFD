...
from core.performance import db_monitor, health_monitor, response_cache, rate_limiter
    ...
    return {
        "requests": {
            ...
            "error_rate_percent": round(
                (health_monitor.error_count / max(health_monitor.request_count, 1)) * 100, 2
            ),
            "uptime_seconds": round(datetime.now().timestamp() - health_monitor.start_time, 2)
        },
        "database": {
            "total_queries": len(db_monitor.query_stats),
            "slow_queries": len([
                q for q, stats in db_monitor.query_stats.items()
            ...
            "average_query_time_ms": round(
                ...
                max(len(db_monitor.query_stats), 1) * 1000, 2
            )
        },
...
async def invalidate_cache_endpoint(
    ...
    try:
        ...
        logger.info(f"Cache invalidation by user {current_user['id']}: {invalidated_count} keys matching '{pattern}'")
    ...
    try:
        ...
        return {
            "rate_limits": {
                ...
                "active_limits": sorted(active_limits, key=lambda x: x["current_requests"], reverse=True)
            },
            "timestamp": datetime.utcnow().isoformat()
        }
    except Exception as e:
...
async def comprehensive_health_check() -> Dict[str, Any]:
    ...
    try:
        ...
        overall_healthy = (
            health_status.get("database", {}).get("healthy", False) and
            redis_healthy and
            health_status["performance_summary"]["error_rate"] < 5.0  # Less than 5% error rate
        )
