...
class ComponentBindingRequest(BaseModel):
    ...
    binding_type: str = Field(..., description="Type of binding: 'library', 'instance', 'specification'")
    location_path: str = Field(..., description="JSONPath to location in document")
    metadata: Optional[Dict[str, Any]] = Field(None, description="Additional binding metadata")
...
class ProjectComponentRequest(BaseModel):
    ...
    project_document_id: str = Field(..., description="UUID of the project ODL-SD document")
    components: List[Dict[str, Any]] = Field(..., description="List of component instances to add")
    location: str = Field(default="libraries.components", description="Location in ODL-SD document")


class ComponentSpecificationRequest(BaseModel):
    """Request model for updating component specifications in documents."""
    component_id: str
    specifications: Dict[str, Any] = Field(..., description="Component specifications to add/update")
    merge_strategy: str = Field(default="deep_merge", description="How to merge specifications")


# Component-Document Integration

@router.post("/bind", response_model=ComponentBindingResponse, status_code=status.HTTP_201_CREATED)
...
@router.get("/components/{component_id}/bindings", response_model=List[ComponentBindingResponse])
...
def create_component_reference(component: Component, binding_type: str, metadata: Optional[Dict[str, Any]]) -> Dict[str, Any]:
...
def create_odl_component_instance(component: Component, instance_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    instance = {
        ...
        "domain": component.domain,
        "scale": component.scale,
        "instance_id": instance_data.get("instance_id", f"INST-{uuid.uuid4().hex[:8].upper()}"),
