...
def upgrade() -> None:
    ...
    # Create base tables first (needed for foreign key constraints)
    # Check if tenants table exists before creating
    ...
    if "users" not in inspector.get_table_names():
        op.create_table(
            ...
            sa.Column("roles", postgresql.JSONB(astext_type=Text()), nullable=True),
            sa.PrimaryKeyConstraint("id"),
        )
...
def downgrade() -> None:
    ...
    # Check if tables exist before dropping (conditional downgrade)
    connection = op.get_bind()
    inspector = sa.inspect(connection)
